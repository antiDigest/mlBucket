...
class Dataset():
    ...
    def getAttributeCount(self, attribute):
        posCount = float(len(self.data[self.data[attribute] == 1]))
        negCount = float(len(self.data[self.data[attribute] == 0]))
        totalCount = float(len(self.data[attribute]))
